...
import { initDatabase, checkDatabaseHealth, closeDatabase } from './db/sqlite.js';
...
app.post('/api/auth/signup', async (req: Request, res: Response) => {
  try {
    ...
    if (existingUser) {
      createAuthLog(undefined, 'signup_failure', getClientIp(req), req.headers['user-agent'] || undefined, false);
    ...
    // Log
    createAuthLog(user.id, 'signup', getClientIp(req), req.headers['user-agent'] || undefined, true);

    // Create session
    const session = createSession(user.id, generateToken(64), new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), getClientIp(req));

    // Set cookie
    res.cookie('humanoid_session', session.token, {
      httpOnly: true,
      path: '/',
...
app.post('/api/auth/verify-email', (req: Request, res: Response) => {
  ...
  if (user) {
    ...
    createAuthLog(user.id, 'email_verification', getClientIp(req), req.headers['user-agent'] || undefined, true);
...
app.post('/api/auth/signin', async (req: Request, res: Response) => {
  try {
    ...
    if (!user) {
      createAuthLog(undefined, 'signin_failure', getClientIp(req), req.headers['user-agent'] || undefined, false);
      return res.status(401).json({ error: 'Invalid credentials', code: 'INVALID_CREDENTIALS' });
    }

    if (!user.emailVerified) {
      createAuthLog(user.id, 'signin_failure', getClientIp(req), req.headers['user-agent'] || undefined, false);
    ...
    const session = createSession(user.id, generateToken(64), expiresAt, getClientIp(req), req.headers['user-agent'] || undefined);

    // Update last signin
    updateUser(user.id, { lastSigninAt: new Date() });

    // Log
    createAuthLog(user.id, 'signin', getClientIp(req), req.headers['user-agent'] || undefined, true);

    // Set cookie
    const maxAge = rememberMe ? 2592000000 : 604800000; // 30 days or 7 days
    res.cookie('humanoid_session', session.token, {
      httpOnly: true,
      path: '/',
      maxAge: maxAge,
...
app.post('/api/auth/signout', (req: Request, res: Response) => {
  ...
  if (token) {
    ...
    if (session) {
      createAuthLog(session.userId, 'signout', getClientIp(req), req.headers['user-agent'] || undefined, true);
      deleteSession(token);
    }
  }

  res.clearCookie('humanoid_session', { path: '/' });
...
app.post('/api/auth/password/reset', (req: Request, res: Response) => {
  ...
  if (user) {
    ...
    createAuthLog(user.id, 'password_reset_request', getClientIp(req), req.headers['user-agent'] || undefined, true);
...
app.post('/api/auth/password/reset/confirm', async (req: Request, res: Response) => {
  ...
  createAuthLog(resetToken.userId, 'password_reset_success', getClientIp(req), req.headers['user-agent'] || undefined, true);
